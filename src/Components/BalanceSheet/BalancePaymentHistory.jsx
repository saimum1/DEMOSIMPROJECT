...
const BalancePaymentHistory = ({actioncallset,agentid,optiontype}) => {
    ...
    const [agentFinancialDataOthersHistory, setagentagentFinancialDataOthersHistory] = useState(null);
    ...
    const getTotaldue = async () => {
        ...
        try {
            ...
            const response = await axios.get(`${config.apiUrl}/api/financial/sim/agents/${agentid}`,{
            headers: {
                Authorization: `Bearer ${token}`
            }
            })
    ...
    const getAgentFinancail = async () => {
        try {
            ...
            setagentFinancialData(response?.data)
    ...
    const deleteSim = async () => {
        try {
            await axios.delete(`${config.apiUrl}/api/financial/sim/agents/${selecteditem?.agentFinanceId}/${status === 'paymentHistory'? 'payment':status === 'sellHistory' ?'simSell': status === 'othersTransectionHistory'?'others':""}/${selecteditem?.id}`,{
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    return (
        <div  className=" h-full w-full  border-none" style={{display:'flex',justifyContent:'center',alignItems:'center',flexDirection:'column',gap:''}}>
            ...
            <AddedPayment data={selecteditem}  isOpen={isOpen} onClose={onClose} agentId={agentid} action={actiontype}  getdata={updateAll} optiontype={optiontype}/>
            <AddTransaction statustype={status} data={selecteditem} isOpen={isOpenUp} onClose={onCloseUp} agentId={agentid} action={actiontype}  getdata={updateAll} optiontype={optiontype}/>
            ...
            <div  style={{padding:".8rem 0",height:'100%',display:'flex',width:'100%',justifyContent:'space-between',alignItems:'center',backgroundColor:'',color:'white',borderRadius : global_css.card_border_radius}}>
                ...
                <div style={{display:'flex',justifyContent:"space-between",alignItems:"flex-start",flexDirection:'column',height:'100%',gap:'.7rem'}}>
                    ...
                    <span style={{fontSize:'32px',fontWeight:700}}>{agentFinancialData?.agentName}</span>
                    {user.role !== 'ADMIN' && 
                    <div style={{display:'flex',justifyContent:"space-between",alignItems:"space-between",flexDirection:'column',height:'100%'}}>
                                        ...
                                        <span>
                                            <FormControl>
                                                ...
                                                <div style={{width:'100%'}}> 
                                                <select style={{border : '1px solid #595959'}} className="w-[17rem] h-10 cursor-pointer bg-[#404040] hover:bg-[#545454] text-[#9CA3AF]  py-2.5 px-4 rounded-md shadow-sm hover:shadow-md transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" onChange={(e) => actioncallset('',profileInfo?.agentId,e.target.value)} value={optiontype}>
                                                    <option value="SIM">
                                                            SIM
                                                        </option>
                    ...
                    <div style={{display:'flex',gap:'1rem'}}>
                        <span style={{border:'1px solid white',borderRadius : global_css.card_border_radius,borderColor:'#fff9',height:'100%',width:'17rem',padding:'.5rem',display:"flex",justifyContent:'center',alignItems:'flex-start',flexDirection:'column'}}>
                            ...
                            <span style={{fontSize:'30px',fontWeight:'700',color:'#FFA526'}}>€ {totalDueSim}</span>
                        </span>
                    </div>}
                ...
                <div style={{display:'flex',justifyContent:"space-between",alignItems:"center",flexDirection:'column',height:'100%',gap:''}}>
                    ...
                    <span>Last payment: {moment(agentFinancialData?.lastSIMTransactionDate).format(' MM/DD/YY')}</span>
                    {user?.role ===  'ADMIN' ?
                    <span  style={{display:'flex',justifyContent:'flex-end',alignItems:'center',flexDirection:'column',gap:'18px',height:"100%"}}>  
                          <button onClick={() => { onOpen(); setactiontype(false)}} className="py-2 px-2 bg-[#27CF7A] text-white font-bold rounded rounded-1xl w-[17rem]">Add payment</button>
                          <button onClick={() => {onOpenUp(); setactiontype(false) }} className="py-2 px-2  text-white font-bold rounded rounded-1xl w-[17rem]" style={{border:'1px solid #999'}}>Add Transection</button>
                    </span>:
                     <div style={{display:'flex',gap:'1rem'}}>
                     <span style={{border:'1px solid white',borderRadius : global_css.card_border_radius,borderColor:'#fff9',height:'100%',width:'17rem',padding:'.5rem',display:"flex",justifyContent:'center',alignItems:'flex-start',flexDirection:'column'}}>
                         <span>Total Payable Due</span>
                          ...
                          { status === 'paymentHistory'?
                            <Table className='h-[45vh]' >
                                ...
                                <TableBody  >
                                    ...
                                    {agentFinancialData?.simTransactions?.payment?.map((item,index) => (
                                        <TableRow key={item.id} style={{borderColor:'#595959'}}>
                                            ...
                                            <TableCell>
                                                <Text>{convertString(item?.paymentType)}</Text>
                                            </TableCell>
                           ...
                           : 
                           status === 'sellHistory' || status === 'othersTransectionHistory'  ? 
                            ...
                            <Table className='h-[45vh]' >
                                ...
                                <TableHead> 
                                    <TableRow className="!bg-[#444444] !rounded !rounded-1xl" style={{margin:'0px 4px',backgroundColor:'red'}}>
                                        ...
                                        <TableHeaderCell style={{borderRight:'2px solid #303038'}}>Sell Amount</TableHeaderCell>
                                        {user?.role === 'ADMIN' ?   <TableHeaderCell style={{borderTopRightRadius:'5px',borderBottomRightRadius:'5px'}}>Action</TableHeaderCell>:''}
                                    </TableRow>
                            ...
                            {status === 'sellHistory' ?
                                ...
                                <TableBody  >
                                    ...
                                    { agentFinancialData?.simTransactions?.simSell?.map((item,index) => (
                                        <TableRow key={item.id} style={{borderColor:'#595959'}}>
                                            ...
                                            <TableCell style={{
                                                ...
                                                {/* <Text>{ item?.operators?.map((n)=>{
                                                    return(
                                                        <span>{n},</span>
                                                    )
                                                })}</Text> */}
                                                <Text>{item?.operators?.join(", ")}</Text>
                                            </TableCell>
                                            ...
                                            <TableCell style={{
                                                ...
                                                {/* <Text>{ item?.simNumbers?.map((n)=>{
                                                    return(
                                                        <span>{n}</span>
                                                    )
                                                })}</Text> */}
                                                <Text>{item?.simNumbers?.join(", ")}</Text>
                            ...
                            status === 'othersTransectionHistory' ?
                                ...
                                { agentFinancialData?.simTransactions?.others?.map((item,index) => (
                                    <TableRow key={item.id} style={{borderColor:'#595959'}}>
