...
const StockReportComponent = ({setList,tableData, setTableData,setCourierForm,loader, setLoader,  setNodata, nodata, agent, startDate, enddate, searchCategory}) => {
    ...
    return (
        <div   className="border-none h-full w-full">
            ...
            <ViewStockReport  isOpen={isOpenoff} onClose={onCloseoff} data={tableData} selecteditem={selecteditem} view={view} setView={setView} setselecteditem={setselecteditem} agent={agent} startDate={startDate} endDate={enddate} searchCategory={searchCategory} selectedDate={selectedDate} setSelectedDate={setSelectedDate}/>
                    ...
                    <Table className="mt-4 h-[60vh] w-full">
                        <TableHead>
                            <TableRow className="!bg-[#444444] !rounded !rounded-1xl">
                                ...
                                <TableHeaderCell style={{ borderRight: '2px solid #303038' }}>In Stock</TableHeaderCell>
                                <TableHeaderCell style={{ borderRight: '2px solid #303038' }}>Active & Sales</TableHeaderCell>
                                <TableHeaderCell style={{ borderRight: '2px solid #303038' }}>Stocks</TableHeaderCell>
                        ...
                        <TableBody style={{ minHeight: '200px' }}>
                                    ...
                                    {tableData?.map((table, i) => (
                                        ...
                                        >
                                            ...
                                            <TableCell>{table?.InStock}</TableCell>
                                            <TableCell>{table?.ActiveSales}</TableCell>
                                            <TableCell>{table?.TotalStocks}</TableCell>
                                            <TableCell>
                                                <div style={{ display: 'flex', gap: '6%', alignItems: 'center', marginLeft: '8%' }}>
                                                    <Eye style={{cursor : 'pointer'}} onClick={() => {setView(true); setselecteditem(table?.OperatorName); onOpenoff()}}/>
                                                    <svg style={{cursor : 'pointer'}} onClick={() => {setView(false); setselecteditem(table?.OperatorName); onOpenoff()}} xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 16 16" fill="none">
