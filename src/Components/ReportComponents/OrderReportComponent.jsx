...
import ViewOrderReport from "./ViewOrderReport.jsx";

const OrderReportComponent = ({setList,tableData, setTableData,setCourierForm,loader, setLoader,  setNodata, nodata, agent, startDate, enddate, searchCategory}) => {
    ...
    return (
        <div   className="border-none h-full w-full">
            ...
            <ViewOrderReport  isOpen={isOpenoff} onClose={onCloseoff} data={tableData} selecteditem={selecteditem} view={view} setView={setView} setselecteditem={setselecteditem} agent={agent} startDate={startDate} endDate={enddate} searchCategory={searchCategory} selectedDate={selectedDate} setSelectedDate={setSelectedDate}/>
                    ...
                    <Table className="mt-4 h-[60vh] w-full">
                        <TableHead>
                            <TableRow className="!bg-[#444444] !rounded !rounded-1xl">
                                ...
                                <TableHeaderCell style={{ borderRight: '2px solid #303038' }}>Total Order</TableHeaderCell>
                        ...
                        <TableBody style={{ minHeight: '200px' }}>
                            {tableData?.map((data, index) => (
                                <React.Fragment key={index}>
                                    ...
                                    {/* Iterate through OrderDetails for vertical display */}
                                    {data?.OrderDetails?.map((order, i) => (
                                        <TableRow key={`${data?.date}-${i}`} style={{'border' : 'none'}}>
                                            {/* Display the date only for the first row */}
                                            {i === 0 && (
                                                <TableCell rowSpan={data?.OrderDetails?.length} style={{ verticalAlign: 'top' }}>
                                                    {moment(data?.date).format('YYYY-MM-DD')}
                                                </TableCell>
                                            )}
                                            <TableCell>{order?.OperatorName}</TableCell>
                                            {/* Display TotalOrders and TotalAmount only for the first row */}
                                            {i === 0 && (
                                                <>
                                                    <TableCell rowSpan={data?.OrderDetails?.length}>
                                                        Є{data?.TotalOrders}
                                                    </TableCell>
                                                    <TableCell rowSpan={data?.OrderDetails?.length}>
                                                        Є{data?.TotalAmount}
                                                    ...
                                                    <TableCell rowSpan={data?.OrderDetails?.length}>
                                                        <div style={{ display: 'flex', gap: '6%', alignItems: 'center', marginLeft: '8%' }}>
                                                            <Eye
                                                                ...
                                                                onClick={() => {
                                                                    setView(true);
                                                                    setselecteditem(order?.OperatorName);
